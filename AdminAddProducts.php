...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
	...
	if (empty($description_err) && empty($images_err) && empty($genres_err) && empty($title_err) && empty($price_err)) {
		...
		if ($productID != null) {
			$game_err = "Game already exists!";
			$added = false;
		} else {
			$success = insertProduct($link, $title, $description, $price);
			if ($success) {
				$productID = getProductID($link, $title);
				if ($productID != null) {
					...
					print "There was a problem getting the product ID";
					$added = false;
				}
			} else {
...
function insertProduct($link, $title, $description, $price)
{
	$sql = "INSERT INTO products (title, description, price, featured) VALUES (?, ?, ?, 0)";
	if ($stmt = mysqli_prepare($link, $sql)) {
		mysqli_stmt_bind_param($stmt, "ssd", $title, $description, $price);
								...
								<div class="col-md-9">
									<div id="innerWrapper">
										<div class="inputDiv">
											...
											<br>
											<p>To use the API check if the product exists <a href="https://rawg.io/games" target="_blank">here</a></p>
										...
										<table>
											...
											if ($genres != null) {
												while ($row = mysqli_fetch_array($genres, MYSQLI_ASSOC)) {
													echo '<tr>';
													echo '<td class="tableCheckbox"><input type="checkbox" name="genres[]" value="' . $row["genre"] . '"></td>';
